...
procedure merge_sub(var l1, l2, l: List);
var tmp, preTail: List;
begin
  while (l1 <> nil) and InList(l2, l1^.val) do begin
    tmp := l1;
    l1 := l1^.next;
    dispose(tmp);
  end;
  ...
  if l1 <> nil then begin
    ...
    while l1 <> nil do
      if not InList(l, l1^.val) and not InList(l2, l1^.val) then begin
        preTail^.next := l1;
        l1 := l1^.next;
